<?php
// ============================================================
//  Creamy Bite – Promo Code Handler
...
if ($action === 'validate') {
    ...
    // Compute the basket value from the SESSION, never from the request.
    // cart_total used to be taken straight off the query string, so sending
    // cart_total=9999 satisfied any minimum-order rule and sized a percentage
    // discount off a number the customer chose.
    $cartTotal = 0.0;
    ...
    if (!empty($_SESSION['trade_user'])) {
        echo json_encode(['success' => false, 'message' => 'Promo codes cannot be applied to wholesale trade accounts.']);
        exit;
    }
    ...
    $_SESSION['promo'] = [
        ...
        'label'           => $discountLabel,
        // min_order must be carried: checkout.php re-checks it after a page
        // reload so a basket that has since shrunk below the threshold drops
        // the discount instead of displaying one the server will refuse.
