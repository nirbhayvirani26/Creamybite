...
try {
    ...
    // Auto-seed/update authentic 13 Creamy Bite products if not present or if using old sample products
    $checkRajbhog = (int)$pdo->query("SELECT COUNT(*) FROM products WHERE name = 'Rajbhog Ice Cream Tub'")->fetchColumn();
    if ($checkRajbhog === 0) {
        $flavorProducts = [
    ...
    // Purge any old sample products that are not real Creamy Bite products
    $realNames = [
        'Rajbhog Ice Cream Tub', 'Kesar Pista Ice Cream Tub', 'Afghan Meva Ice Cream Tub',
        'Roasted Almond Ice Cream Tub', 'American Dry Fruits Tub', 'Kaju Draksh Ice Cream Tub',
        'Fresh Sitafal (Custard Apple)', 'Naked Coconut Ice Cream Tub', 'Kaju Gulkand Ice Cream Tub',
        'Mava Malai Ice Cream Tub', 'Cookies & Cream Tub', 'Choco Chips Ice Cream Tub',
        'Pan Masala Ice Cream Tub'
    ];
    $inClause = implode(',', array_fill(0, count($realNames), '?'));
    $delStmt = $pdo->prepare("DELETE FROM products WHERE name NOT IN ($inClause)");
    $delStmt->execute($realNames);
...
<div class="mobile-drawer" id="mobileDrawer">
    <div class="mobile-nav-panel">
        ...
        <div class="mobile-nav-actions">
            <button class="btn-view-cart" onclick="closeMobileMenu(); openCart();" style="width:100%; justify-content:center;">
                <i class="fa-solid fa-bag-shopping"></i> View Cart
            </button>
            <a href="checkout.php" class="btn-primary" style="justify-content:center; display:flex;">
...
<!-- ══ Products / Menu ═════════════════════════════════════ -->
<section class="products-section" id="menu" style="padding-top:48px;">
    <div class="container">
        <?php
        $isTradeUser = !empty($_SESSION['trade_user']);
        if ($isTradeUser):
        ?>
        <div style="background:linear-gradient(135deg, var(--color-primary), var(--color-primary-dark)); color:white; padding:14px 20px; border-radius:var(--radius-md); margin-bottom:28px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; box-shadow:var(--shadow-md);">
            <div>
                <i class="fa-solid fa-store" style="font-size:18px; margin-right:8px; color:var(--color-secondary);"></i>
                <strong>Trade Wholesale Active:</strong> <?= htmlspecialchars($_SESSION['trade_user']['business_name']) ?>
                <span style="font-size:12px; opacity:0.85; margin-left:6px;">(Wholesale pricing applied)</span>
            </div>
            <a href="trade_logout.php" class="btn-secondary" style="padding:6px 14px; font-size:12px; background:rgba(255,255,255,0.15); color:white; border:1px solid rgba(255,255,255,0.3);">
                <i class="fa-solid fa-right-from-bracket"></i> Logout Trade
            </a>
        </div>
        <?php else: ?>
        <div style="background:var(--color-primary-bg); border:1px solid rgba(199,136,41,0.25); padding:10px 18px; border-radius:12px; margin-bottom:24px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; font-size:13px;">
            <div>
                <i class="fa-solid fa-store" style="color:var(--color-primary); margin-right:6px;"></i>
                <strong>Are you a Shop, Cafe, or Restaurant owner?</strong> Get wholesale pricing on bulk tubs.
            </div>
            <div style="display:flex; gap:12px; font-size:12px;">
                <a href="trade_register.php" style="color:var(--color-primary); font-weight:700; text-decoration:none;">Apply for Trade &rarr;</a>
                <span style="opacity:0.4;">|</span>
                <a href="trade_login.php" style="color:var(--color-primary); font-weight:700; text-decoration:none;">Trade Login &rarr;</a>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="products-grid" id="productsGrid">
            ...
            <article class="product-card glass-panel<?php if ($isOutOfStock): ?> out-of-stock<?php endif; ?>"
                ...
                <div class="product-image-wrap">
                    ...
                    <span class="product-image-placeholder" style="display:none;"><?= htmlspecialchars($product['emoji'] ?? '🍦') ?></span>
                    <?php else: ?>
                ...
                <div class="product-body">
                    ...
                    <div class="product-footer">
                        <div>
                            ...
                            ?>
                            <div class="product-price-from" style="color:var(--color-secondary); font-weight:700;">Trade Wholesale</div>
                            <div class="product-price">£<?= number_format($wPrice, 2) ?> <s style="font-size:12px; color:var(--text-muted); font-weight:400;">£<?= number_format($product['price'], 2) ?></s></div>
                            ...
                            <div style="font-size:11px; color:#f59e0b; font-weight:700; margin-top:2px;">⚠️ Only <?= (int)$product['stock_qty'] ?> left!</div>
                            <?php endif; ?>
                            <!-- In-cart indicator -->
                        ...
                        <button class="btn-add-cart" disabled
                            style="background:var(--bg-surface); border:1.5px solid var(--border-color); color:var(--text-muted); cursor:not-allowed; font-size:10px; width:auto; padding:0 10px; font-weight:700; letter-spacing:0.3px;"
...
<aside class="cart-sidebar" id="cartSidebar">
    <div class="cart-header">
        <h2><i class="fa-solid fa-bag-shopping" style="color:var(--color-primary);"></i> Your Order</h2>
...
<div id="deliveryRadiusPopup" style="display:none; position:fixed; bottom:28px; right:28px; max-width:390px; background:linear-gradient(135deg, #5C1D24, #3d1318); color:white; padding:20px 22px; border-radius:18px; box-shadow:0 14px 40px rgba(0,0,0,0.35); z-index:99999; border:1px solid rgba(199,136,41,0.4);">
    <div style="display:flex; align-items:flex-start; gap:14px;">
        <div style="font-size:32px; line-height:1; flex-shrink:0;">🚚</div>
        <div style="flex:1;">
            <strong style="font-size:15px; display:block; margin-bottom:4px; color:#C78829; font-weight:800;">
                📍 Harrow Delivery Area Notice
            </strong>
            <p style="font-size:13px; margin:0 0 10px; line-height:1.45; color:rgba(255,255,255,0.92);">
                We deliver fresh handcrafted ice cream within a <strong>6-mile radius of Harrow (HA1 4EX / HA1 2SP)</strong>.<br>
                <span style="color:#a7f3d0; font-weight:700;">🎉 Free delivery under 3 miles!</span>
            </p>
            <div style="font-size:12px; opacity:0.85;">
                <i class="fa-solid fa-store" style="color:#C78829;"></i> Warehouse collection is also available!
            </div>
        </div>
        <button onclick="closeDeliveryPopup()" style="background:rgba(255,255,255,0.18); border:none; color:white; width:26px; height:26px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:14px; flex-shrink:0;">
            <i class="fa-solid fa-xmark"></i>
        </button>
    </div>
</div>
