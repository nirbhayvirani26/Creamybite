        ...
        try {
            if ($productId > 0) {
                ...
                $stmt = $pdo->prepare("UPDATE products SET name=:name, description=:description, price=:price, wholesale_price=:wholesale_price,
                    ...
                    WHERE id=:id");
                $stmt->execute(compact('name','description','price','wholesale_price','category','emoji','badge','available') + ['image' => $imageName, 'nuts_allergy' => $nuts_allergy, 'track_stock' => $track_stock, 'stock_qty' => $stock_qty, 'id' => $productId]);
                header('Location: index.php?tab=products&product_updated=1'); exit;
            } else {
                // INSERT
                $stmt = $pdo->prepare("INSERT INTO products (name, description, price, wholesale_price, category, emoji, image, badge, available, nuts_allergy, track_stock, stock_qty)
                    VALUES (:name, :description, :price, :wholesale_price, :category, :emoji, :image, :badge, :available, :nuts_allergy, :track_stock, :stock_qty)");
                $stmt->execute(compact('name','description','price','wholesale_price','category','emoji','badge','available') + ['image' => $imageName, 'nuts_allergy' => $nuts_allergy, 'track_stock' => $track_stock, 'stock_qty' => $stock_qty]);
...
<main style="padding:40px 0 80px;">
    <div class="container" style="max-width:900px;">
        ...
        <div class="admin-page-header">
            ...
            <a href="index.php?tab=products" class="btn-secondary" style="padding:10px 18px; font-size:13px;">
        ...
        <form action="product_form.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="product-form-grid" style="display:grid; grid-template-columns:1fr 300px; gap:28px; align-items:start;">
                ...
                <div>
                    <div class="glass-panel form-section">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                <label class="form-label">Wholesale Price (£) <small style="color:var(--color-primary); font-weight:700;">(Trade B2B)</small></label>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group" style="display:flex; align-items:flex-end; padding-bottom:4px;">
                                <label style="display:flex; align-items:center; gap:12px; cursor:pointer; font-size:15px; font-weight:600; color:var(--text-secondary);">
                                    <input type="checkbox" name="available" value="1" style="width:18px; height:18px; accent-color:var(--color-primary);"
                                        <?= ($product['available'] ?? 1) ? 'checked' : '' ?>>
                                    Available on menu
                                </label>
                            </div>
                            <div class="form-group" style="display:flex; align-items:flex-end; padding-bottom:4px;">
                                <label style="display:flex; align-items:center; gap:12px; cursor:pointer; font-size:15px; font-weight:600; color:#b45309;">
                                    <input type="checkbox" name="nuts_allergy" value="1" style="width:18px; height:18px; accent-color:#b45309;"
                    ...
                    <div class="form-row" style="margin-top:8px;">
                        <div class="form-group" style="display:flex; align-items:flex-end; padding-bottom:4px;">
                            <label style="display:flex; align-items:center; gap:12px; cursor:pointer; font-size:15px; font-weight:600; color:var(--text-secondary);">
                                <input type="checkbox" name="track_stock" value="1" id="trackStockCb" style="width:18px; height:18px; accent-color:var(--color-primary);"
                                    <?= !empty($product['track_stock']) ? 'checked' : '' ?>>
                                📦 Track Stock
                            </label>
                            <?php if ($displayInStock !== null): ?>
                            <span style="margin-left:16px; font-size:13px; background:rgba(16,185,129,0.12); color:#10b981; border-radius:20px; padding:3px 12px; font-weight:700;">
                    ...
                    <div class="glass-panel form-section">
                        ...
                        <div style="margin-bottom:16px;">
                            <p style="font-size:12px; color:var(--text-muted); margin-bottom:8px;">Current image:</p>
                            <img src="../assets/images/products/<?= htmlspecialchars($product['image']) ?>"
                                 alt="Current product image"
                                 class="image-current-thumb"
                        ...
                        <img src="" alt="New image preview" id="newImgPreview"
                             style="display:none; max-height:180px; border-radius:var(--radius-sm); margin-top:10px; border:1.5px solid var(--border-light);">

                        <!-- Fallback emoji (shown if no image) -->
                        <div class="form-group" style="margin-top:14px;">
                            <label class="form-label" style="font-size:11px;">Emoji (shown if no image)</label>
                            <input type="text" name="emoji" id="emojiInput" class="form-control"
                                value="<?= htmlspecialchars($product['emoji'] ?? '🍦') ?>"
                                style="font-size:22px; max-width:100px;">
                        </div>
                    </div>
                ...
                <div style="position:sticky; top:88px;">
                    <div class="glass-panel preview-card">
                        <p style="font-size:11px; text-transform:uppercase; letter-spacing:0.1em; color:var(--text-muted); margin-bottom:12px;">Live Preview</p>
                        <div class="preview-img-wrap" id="previewWrap">
                            ...
                            <?php else: ?>
                            <span id="previewEmoji" style="font-size:64px;"><?= htmlspecialchars($product['emoji'] ?? '🍦') ?></span>
                            <?php endif; ?>
                        </div>
                        <h3 id="previewName" style="font-size:17px; margin-bottom:6px;"><?= htmlspecialchars($product['name'] ?? 'Product Name') ?></h3>
                        <div id="previewPrice" style="font-size:22px; font-weight:800; font-family:var(--font-heading); color:var(--color-primary); margin-top:4px;">
        ...
        <div class="glass-panel form-section" style="margin-top:28px;">
            <h3 style="display:flex; align-items:center; gap:10px;">
                <i class="fa-solid fa-tags" style="color:var(--color-secondary);"></i>
                Variants / Sizes
                <span style="font-size:12px; font-weight:500; color:var(--text-muted); margin-left:4px;">Optional — e.g. 500ml, 1L, Small, Large</span>
            </h3>
            ...
            <div style="display:flex; align-items:center; gap:12px; margin-top:14px; padding:14px 16px; border:2px dashed var(--border-strong); border-radius:var(--radius-sm); background:var(--bg-surface); flex-wrap:wrap;">
                ...
                <input type="text" id="newVariantName" placeholder="Size name (e.g. 500ml, 1L, Small)" class="form-control" style="flex:1; min-width:140px;">
                <div style="display:flex; align-items:center; gap:4px;">
                    <span style="font-size:12px; font-weight:700; color:var(--text-secondary);">Retail: £</span>
                    <input type="number" id="newVariantPrice" placeholder="0.00" step="0.01" min="0.01" class="form-control" style="width:85px;">
                </div>
                <div style="display:flex; align-items:center; gap:4px;">
                    <span style="font-size:12px; font-weight:700; color:var(--color-primary);">Trade: £</span>
                    <input type="number" id="newVariantWholesalePrice" placeholder="0.00" step="0.01" min="0.00" class="form-control" style="width:85px;">
                </div>
...
function updateVariant(id, name, price, wholesalePrice, available) {
    ...
    updateTimer[id] = setTimeout(() => {
        fetch('variant_handler.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `action=update&id=${id}&product_id=0&name=${encodeURIComponent(name)}&price=${parseFloat(price)||0}&wholesale_price=${parseFloat(wholesalePrice)||0}&${available ? 'available=1' : ''}`,
        })
        .then(r => r.json())
        .then(data => {
